		...
		if(IsPlayerInAnyVehicle(playerid))
		{
			...
			if(newstate == PLAYER_STATE_DRIVER)
			{
				if(Araclar[aracid][AracTur] > 0)
				{
					...
					{
						SunucuEx(playerid, "{3C5DA5}[SÜRÜCÜ KURSU] Sürüþ testiniz baþladý. Ýþaretli noktalarý takip edin, hýz sýnýrý bulunmamaktadýr.");
...
public OnPlayerText(playerid, text[])
{
	...
	if(!strcmp(text, ":/", true))
	{
		cmd(playerid, 1, "aðzýný yamultur.");
		return false;
	}
	if(!strcmp(text, ";)", true))
	{
...
public OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
	if(newkeys == KEY_YES)
	{
		...
		if(BinaYakin(playerid) != -1)
		{
			...
			if(Binalar[binaid][Durum] == 1) return Sunucu(playerid, "Bu bina kilitli. Giriþ yapamazsýnýz.");
			if(Binalar[binaid][Int][0] == 0.0 && Binalar[binaid][Int][1] == 0.0 && Binalar[binaid][Int][2] == 0.0) return Sunucu(playerid, "Bu binanýn iç mekaný henüz ayarlanmamýþ görünüyor.");
		...
		if(EvYakin(playerid) != -1)
		{
			...
			if(Evler[evid][Durum] == 1) return Sunucu(playerid, "Bu ev kilitli. Giriþ yapamazsýnýz.");
			if(Evler[evid][Int][0] == 0.0 && Evler[evid][Int][1] == 0.0 && Evler[evid][Int][2] == 0.0) return Sunucu(playerid, "Bu evin iç mekaný henüz ayarlanmamýþ görünüyor.");
		...
		if(IsyeriYakin(playerid) != -1)
		{
			...
			if(Isyeri[isyeriid][Durum] == 1) return Sunucu(playerid, "Bu iþyeri kilitli. Giriþ yapamazsýnýz.");
			if(Isyeri[isyeriid][Int][0] == 0.0 && Isyeri[isyeriid][Int][1] == 0.0 && Isyeri[isyeriid][Int][2] == 0.0) return Sunucu(playerid, "Bu iþyerinin iç mekaný henüz ayarlanmamýþ görünüyor.");
	...
	if(newkeys & KEY_SUBMISSION)
	{
			if(Karakter[playerid][Ciftlik] != -1 && GetPVarInt(playerid, "CiftlikBicimIslem") == 1)
			{
				if(GetPlayerProgressBarValue(playerid, bicimprog[playerid]) >= GetPVarInt(playerid, "CiftlikEkinler"))
				{
					...
					Sunucu(playerid, "Baþarýyla tarlanýzda ekili olan %s bitkisini biçtiniz.", ekinlerveri[CiftlikInfo[cid][cUrun]][ekinisim]);
					Doluluk[playerid] = 0;
...
public OnPlayerCommandPerformed(playerid, cmd[], params[], result, flags)
{
    if(result == -1)
    {
        Sunucu(playerid, "Sunucuda böyle bir komut bulunmamaktadýr.");
        return 0;
    }
    return 1;
}
...
{
	if(!Karakter[playerid][Giris]) return Sunucu(playerid, "Giriþ yapmadan komut kullanamazsýnýz."), 0;
	...
	if(Karakter[playerid][EhliyetTesti] == 1 && Araclar[aracid][AracTur] == 1)
	{
		switch(Karakter[playerid][EhliyetCP]) 
		{
			...
			case 8: 
			{
				...
				{
					Sunucu(playerid, "Hile þüphesi nedeniyle sýnav iptal oldu. Lütfen tekrar deneyin.");
			...
			case 17:
			{
				...
				if(aracHp < 750.0)
				{
					Sunucu(playerid, "Araca çok fazla hasar verdiðinizden dolayý sýnavý geçemediniz.");
					Karakter[playerid][EhliyetTesti] = 0;
					AracSpawnla(aracid);
					new Float:x, Float:y, Float:z;
					GetVehiclePos(aracid, Float:x, Float:y, Float:z);
				...
				{
					Sunucu(playerid, "Sýnavý baþarýyla geçtiniz. Artýk ehliyet sahibisiniz.");
					Karakter[playerid][EhliyetTesti] = 0;
	...
	switch(dialogid)
	{
					...
					case 1:
					{
						switch(Karakter[playerid][Ten])
						{
							case 1:
							{
								...
								Dialog_Show(playerid, DIALOG_KIYAFET_ONAY, DIALOG_STYLE_MSGBOX, "{99C794}Kýyafet Satýn Alma", "{FFFFFF}%d ID'li skini {3C5DA5}%s{FFFFFF} karþýlýðýnda satýn almak istediðinize emin misiniz?\n{3C5DA5}NOT:{FFFFFF} Bu iþlemin geri dönüþü yoktur. Üstünüzdeki kýyafet deðiþecek.", "Onayla", "Ýptal", model, NumaraFormati(KIYAFET_UCRET));
							}
							case 2:
							{
								new model;
		...
		case 23301:
		{
			if(response)
			{
				...
				if(Karakter[playerid][Para] < fiyat) return Sunucu(playerid, "Bu aracý satýn almak için yeterli miktarda paran yok.");
				SetPVarInt(playerid, "AracModel", aracmodel);
				SetPVarInt(playerid, "AracFiyat", fiyat);
				Dialog_Show(playerid, DIALOG_ARAC_SATINAL, DIALOG_STYLE_MSGBOX, "{99C794}Araç Satýn Al", "{3C5DA5}%s{FFFFFF} model aracý {3C5DA5}%s{FFFFFF} karþýlýðýnda satýn almak istiyor musun?", "Onayla", "Ýptal", GetVehicleNameByModel(aracmodel), NumaraFormati(fiyat));
			}
		}
		case 23302:
		{
...
public OnPlayerEnterDynamicArea(playerid, areaid) 
{
	if(Karakter[playerid][Giris])
	{
		for(new i = 0; i < MAX_CIFTLIK; i++) if(CiftlikInfo[i][cArsa] == areaid)
		{
			...
			{
			Sunucu(playerid, "Çiftlik Adý: Florida Eyalet Çiftlikleri (ID: %d)",i);
			...
			Sunucu(playerid, "Bu arsa satýlýk! Arsayý satýn almak için '/satinal'");
			Karakter[playerid][Ciftlik] = i;
			}
			else
			{
			Sunucu(playerid, "Çiftlik Adý: %s (ID: %d)", CiftlikInfo[i][cIsim], i);
			...
			Sunucu(playerid, "Ekili ürün: %s | Ürünlerin olgunlaþmasýna %d saat kaldý.", ekinlerveri[CiftlikInfo[i][cUrun]][ekinisim], CiftlikInfo[i][cOlgunlasma]);
			Karakter[playerid][Ciftlik] = i;
			}
		}
	}
...
public OnPlayerExitVehicle(playerid, vehicleid)
{
	if(GetPVarInt(playerid, "CiftlikEkimIslem") == 1)
 	{
		...
		Sunucu(playerid, "Tarla ekimi sýrasýnda traktörden indiðiniz için ekim iþlemi durdu.");
	}
	if(GetPVarInt(playerid, "CiftlikBicimIslem") == 1)
	{
		...
		Sunucu(playerid, "Tarlayý biçerken biçerdöverden indiðiniz için biçme iþlemi durdu.");
	}
	if(Karakter[playerid][EhliyetTesti] == 1)
	{
		Sunucu(playerid, "Araçtan indiðiniz için sürüþ testi iptal oldu.");
		Karakter[playerid][EhliyetTesti] = 0;
